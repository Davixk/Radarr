...
namespace NzbDrone.Core.MediaFiles
{
    public interface IDownloadedMovieImportService
    {
        ...
        DownloadedMovieImportBatch DecidePath(string path, ImportMode importMode = ImportMode.Auto, Movie movie = null, DownloadClientItem downloadClientItem = null);

        // Mutating COMMIT phase: imports the (already decided) batch. Must run serially and in order.
        List<ImportResult> ImportDecidedBatch(DownloadedMovieImportBatch batch);
    ...
    public class DownloadedMovieImportService : IDownloadedMovieImportService
    {
        ...
        {
            return ImportDecidedBatch(DecidePath(path, importMode, movie, downloadClientItem));
        }

        public DownloadedMovieImportBatch DecidePath(string path, ImportMode importMode = ImportMode.Auto, Movie movie = null, DownloadClientItem downloadClientItem = null)
        {
            ...
            if (_diskProvider.FolderExists(path))
            {
                ...
                var folderMovie = movie ?? _parsingService.GetMovie(GetCleanedUpFolderName(directoryInfo.Name));

                if (folderMovie == null)
                {
                    _logger.Debug("Unknown Movie {0}", GetCleanedUpFolderName(directoryInfo.Name));

                    return EarlyBatch(UnknownMovieResult("Unknown Movie"));
                }

                return DecideFolder(directoryInfo, importMode, folderMovie, downloadClientItem);
            }

            if (_diskProvider.FileExists(path))
            {
                var fileInfo = new FileInfo(path);
                var fileMovie = movie ?? _parsingService.GetMovie(Path.GetFileNameWithoutExtension(fileInfo.Name));

                if (fileMovie == null)
                {
                    _logger.Debug("Unknown Movie for file: {0}", fileInfo.Name);

                    return EarlyBatch(UnknownMovieResult(string.Format("Unknown Movie for file: {0}", fileInfo.Name), fileInfo.FullName));
                }

                return DecideFile(fileInfo, importMode, fileMovie, downloadClientItem);
            }
        ...
        public List<ImportResult> ImportDecidedBatch(DownloadedMovieImportBatch batch)
        {
            ...
            var importResults = _importApprovedMovie.Import(batch.Decisions, true, batch.DownloadClientItem, importMode);
            ...
            if (batch.DirectoryInfo != null)
            {
                if (importMode == ImportMode.Move &&
                    ...
                    ShouldDeleteFolder(batch.DirectoryInfo, batch.Movie))
                {
        ...
        private List<ImportResult> ProcessFolder(DirectoryInfo directoryInfo, ImportMode importMode, DownloadClientItem downloadClientItem)
        {
            ...
            return ImportDecidedBatch(DecideFolder(directoryInfo, importMode, movie, downloadClientItem));
        }
        ...
        private DownloadedMovieImportBatch DecideFolder(DirectoryInfo directoryInfo, ImportMode importMode, Movie movie, DownloadClientItem downloadClientItem)
        {
            if (_movieService.MoviePathExists(directoryInfo.FullName))
            {
                ...
                return EarlyBatch(RejectionResult(ImportRejectionReason.MovieFolder, "Import path is mapped to a movie folder"));
            }
            ...
            return new DownloadedMovieImportBatch
            {
                Decisions = decisions,
                Movie = movie,
        ...
        private List<ImportResult> ProcessFile(FileInfo fileInfo, ImportMode importMode, DownloadClientItem downloadClientItem)
        {
            ...
            return ImportDecidedBatch(DecideFile(fileInfo, importMode, movie, downloadClientItem));
        }
        ...
        private DownloadedMovieImportBatch DecideFile(FileInfo fileInfo, ImportMode importMode, Movie movie, DownloadClientItem downloadClientItem)
        {
            if (Path.GetFileNameWithoutExtension(fileInfo.Name).StartsWith("._"))
            {
                _logger.Debug("[{0}] starts with '._', skipping", fileInfo.FullName);

                return EarlyBatch(new ImportResult(new ImportDecision(new LocalMovie { Path = fileInfo.FullName }, new ImportRejection(ImportRejectionReason.InvalidFilePath, "Invalid video file, filename starts with '._'")), "Invalid video file, filename starts with '._'"));
            }
            ...
            {
                return EarlyBatch(new ImportResult(new ImportDecision(new LocalMovie { Path = fileInfo.FullName },
        ...
        private static DownloadedMovieImportBatch EarlyBatch(params ImportResult[] results)
        {
            return new DownloadedMovieImportBatch
            {
